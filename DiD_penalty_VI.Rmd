...
source("~/Dropbox/Research/VI/vertical-integration/ggplot_smooth_func.R")
...
ggsave("~/Dropbox/Research/VI/output/ppr_realp.pdf", width =15, height=15, units="cm")
...
```{r}
df <- read.csv("~/Dropbox/Research/VI/data/DIDest/trend_bytertile.csv")
...
# & gp!="shref" & gp!="den_nsnf_used")

df$gp <- factor(df$gp, levels=c("read30","read30_pac","shref","vi_snf","refhhi","shref_bytopSNF", "den_nsnf_used","nsnf_used", "comorbidsum","dischnum_pac"), labels = c("30-day readmission","30-day readmission after referred to SNF","Probability of referring to SNF","Acquisition of SNF","SNF referral HHI","Referral share by top referred SNF","# referred SNFs / # referrals","# referred SNFs", "Sum of share of discharges w/ comorbidities", "Number of SNF referrals"))

df$year <- df$fy
df$high <- factor(df$high, level= c(1,0), labels = c("Top tertile","Bottom tertile"))

# just the readmission outcomes alone
df2 <- subset(df, gp=="30-day readmission" | gp=="30-day readmission after referred to SNF")
ggplot(df2) + facet_wrap(~gp, ncol=2) +
  geom_point(aes(x=year, y = mean, shape=high, linetype=high), size=2) +
  geom_line(aes(x=year, y = mean, shape=high, linetype=high), alpha = 0.5) +
  scale_y_continuous(limits=c(.1, .35),
                     breaks = seq(.1,0.35,0.05)) +
...
ggsave("~/Dropbox/Research/VI/output/desc_readmit.pdf", width = 20, height=10, units="cm")

# all outcomes in a single figure:  "Impact of 1% increase in the predicted penalty rate for 2013"
df2 <- subset(df, gp!="30-day readmission" & gp!="30-day readmission after referred to SNF")
ggplot(df2) + facet_wrap(~gp, ncol=2, scale="free") +
  geom_point(aes(x=year, y = mean, shape=high, linetype=high), size=2) +
  geom_line(aes(x=year, y = mean, shape=high, linetype=high), alpha = 0.2) +
  ...
  guides(linetype=guide_legend(title="Penalty pressure"),
         shape=guide_legend(title="Penalty pressure"))
ggsave("~/Dropbox/Research/VI/output/desc_alloutc.pdf", width = 25, height=20, units="cm")
...
# OLS Results: Import coefficient estimates & 95% confidence intervals from the diff-in-diff estimates
```{r}
df <- read.csv("~/Dropbox/Research/VI/data/DIDest/coef_DiD_static.csv")
...
table(df$gp)
df <- subset(df, gp!="lnreqSNF_80pct")
df$gp <- factor(df$gp, levels=c("read30","read30_pac","shref","vi_snf","refhhi","shref_bytopSNF","den_nsnf_used"), labels = c("30-day readmission","30-day readmission after referred to SNF","Share of discharges referred to SNF","Acquisition of SNF","SNF referral HHI","Referral share by top referred SNF","Ratio of # referred SNFs to # referrals"))

# df <- subset(df, gp!="lnreqSNF_80pct" & gp!="den_nsnf_used" & gp!="shref")
# df$gp <- factor(df$gp, levels=c("read30","read30_pac","vi_snf","refhhi","shref_bytopSNF"), labels = c("30-day readmission","30-day readmission after referred to SNF","Acquisition of SNF","SNF referral HHI","Referral share by top referred SNF"))
...
ggplot(df) + facet_wrap(~gp, ncol=2) +
  ...
  theme(legend.position="bottom", 
        ...
        panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank())
ggsave("~/Dropbox/Research/VI/output/OLS_alloutc.pdf", width = 16, height=15, units="cm")

# with reform participation variables
...
# all outcomes in a single figure:  "Impact of 1% increase in the predicted penalty rate for 2013"
ggplot(df) + facet_wrap(~gp, ncol=3) +
  geom_point(aes(x=year, y = estimate), size=2, shape=1) +
  geom_line(aes(x=year, y = estimate), alpha = 0.2) +
  geom_errorbar(aes(x=year, ymin=min95, ymax=max95), width=.1, alpha=0.5) +
  scale_y_continuous(limits=c(-.15, .23),
  ...
  theme(legend.position="bottom", 
        ...
        panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank())
ggsave("~/Dropbox/Research/VI/output/OLS_alloutc_reform.pdf", width = 25, height=15, units="cm")
...
ggplot(df2) +
  ...
  labs(x="Year ending in June",
       y="",
       subtitle="Impact of 1% increase in the predicted penalty rate for 2013 on \n30-day hospital readmission rate") +
  theme(legend.position="bottom", 
        legend.title=element_blank(),
        panel.grid.major.x = element_blank())
ggsave("~/Dropbox/Research/VI/output/readmit.pdf", width = 15, height=15, units="cm")
...
ggplot(df2) +
  ...
  theme(panel.grid.major.x = element_blank())
ggsave("~/Dropbox/Research/VI/output/refhhi.pdf", width = 15, height=10, units="cm")
...
ggplot(df2) +
  ...
  labs(x="Year ending in June",
       y="",
       subtitle="Log Number of SNFs required to account for 80% of SNF referrals") +
  theme(panel.grid.major.x = element_blank())
ggsave("~/Dropbox/Research/VI/output/lnreqSNF_80pct.pdf", width = 15, height=10, units="cm")
...
ggplot(df2) +
  ...
  theme(panel.grid.major.x = element_blank())
ggsave("~/Dropbox/Research/VI/output/shref_bytopSNF.pdf", width = 15, height=10, units="cm")
...
ggplot(df2) +
  ...
  theme(panel.grid.major.x = element_blank())
ggsave("~/Dropbox/Research/VI/output/den_nsnf_used.pdf", width = 15, height=10, units="cm")
...
ggplot(df2) +
  ...
  geom_line(aes(x=year, y = estimate)) +
  geom_errorbar(aes(x=year, ymin=min95, ymax=max95), width=.1, alpha=0.5) +
  scale_y_continuous(limits=c(-.04, 0.09),
  ...
  theme(panel.grid.major.x = element_blank())
ggsave("~/Dropbox/Research/VI/output/vi_snf.pdf", width = 15, height=10, units="cm")
```
