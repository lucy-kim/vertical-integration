...
df <- subset(df, gp!="read30_pac" & gp!="read30_other" & gp!="shrefAMI" & gp!="shrefHF" & gp!="shrefHK" & gp!="shrefPN")
...
range1 <- range(0,0.25)
range2 <- range(0,0.05)
range3 <- range(0.2,0.3)
range4 <- range(30,40)
range5 <- range(6,8)
range6 <- range(-0.2,0.03)
yrrange <- range(2009,2016)

dummy1 <- data.frame(year = yrrange, mean=range0,
                     gp = "A. Discharges referred to SNF, %", stringsAsFactors=TRUE)
dummy2 <- data.frame(year = yrrange, mean=range1,
                     gp = "B. Average comorbidity count per SNF referred patient", stringsAsFactors=TRUE)
dummy3 <- data.frame(year = yrrange, mean=range2,
                     gp = "C. Probability of acquiring SNF", stringsAsFactors=TRUE)
dummy4 <- data.frame(year = yrrange, mean=range3,
                     gp = "D. SNF referral HHI", stringsAsFactors=TRUE)
dummy5 <- data.frame(year = yrrange, mean=range4,
                     gp = "E. Share of referrals to hospital's most referred SNF, %", stringsAsFactors=TRUE)
dummy6 <- data.frame(year = yrrange, mean=range5,
                     gp = "F. Number of SNFs accounting for 80% of referrals", stringsAsFactors=TRUE)
dummy7 <- data.frame(year = yrrange, mean=range6,
                     gp = "G. Average SNF performance at baseline", stringsAsFactors=TRUE)
dummies <- rbind(dummy1,dummy2, dummy3, dummy4, dummy5, dummy6, dummy7)
...
ggplot(df, aes(x=year, y = mean)) + 
  ...
  scale_shape_manual(values = c(17,1)) 
ggsave("~/Dropbox/Research/VI/output/desc_alloutc.pdf", width = 25, height=25, units="cm")
```
...
# df$gp <- factor(df$gp, levels=c("shref","comorbidsum","vi_snf","refhhi3","shref_bytopSNF","lnreqSNF_80pct","qual_read"), labels = c("A. Discharges referred to SNF, %","B. Average comorbidity count per SNF referred patient","C. Probability of acquiring SNF","D. SNF referral HHI","E. Share of referrals to hospital's most referred SNF, %","F. Log Number of SNFs accounting for 80% of referrals","G. Average SNF performance at baseline"))
