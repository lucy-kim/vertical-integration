...
use hosp_fy_VI_cond_nosw, clear
...
merge 1:1 provid fy cond using qualindex_nosw_cond, keep(1 3) nogen
...
use `tmp', clear
merge 1:1 provid fy cond using `cond2', keep(1 3) nogen
...
foreach v of varlist dischnum beds dischnum_pac other_denom {
  bys cond provid: gen `v'1 = `v' if _n==1
  bys cond provid: replace `v'1 = `v'1[_n-1] if `v'1>=.
}
...
/* drop if cond=="HK" */
collapse (sum) _65_74-total, by(provid fy cond)
tempfile agebin
save `agebin'
restore

merge 1:1 provid fy cond using `agebin', keep(1 3) nogen
...
/* drop if cond=="HK" */
collapse (sum) _75_84_pac-_95p_pac, by(provid fy cond)
tempfile agebin2
save `agebin2'
restore

merge 1:1 provid fy cond using `agebin2', keep(1 3) nogen
...
use `tmp2', clear
drop dischrg
merge m:1 provid fy using `cr', keepusing(mcre_dischrg dischrg) nogen keep(1 3)
assert mcre_dischrg!=.
assert dischrg!=.
...
foreach c in "AMI" "HF" "PN" "HK" {
  ...
  loc file1 `c'_`pptype'_pv
  loc file2 `c'_`pptype'_ci
